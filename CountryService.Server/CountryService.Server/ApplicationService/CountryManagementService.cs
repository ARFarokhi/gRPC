...
namespace CountryService.Server.ApplicationService
{
    /// <summary>
    /// async and Task are used: 
    /// for simulateing get data from datebase or datastore
    /// </summary>
    public class CountryManagementService
    {
        ...
        public async Task<IEnumerable<CountryCreationReply>> CreateAsync(List<CountryCreationRequest> countries)
        {
            ...
            var newCountriesResponse = newCountries.Select(x => new CountryCreationReply { Id = x.Id, Name = x.Name }).ToList();

            return await Task.FromResult(newCountriesResponse);
        }

        private void AddCountriesToCollection()
        {
